...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (geminiStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Authenticate Gemini CLI: run `!gemini` once and pick a sign-in method, or set `GEMINI_API_KEY` (Gemini Developer API) / `GOOGLE_API_KEY` (Vertex AI).");
  }
...
function ensureGeminiAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Gemini CLI is not installed or is missing ACP runtime support. Install it with `npm install -g @google/gemini-cli`, then rerun `/gemini:setup`.");
  }
}

function validateReviewRequest(reviewName, focusText) {
  if (reviewName === "Review" && focusText.trim()) {
    throw new Error(
      `\`/gemini:review\` does not take custom focus text. Retry with \`/gemini:adversarial-review ${focusText.trim()}\` to steer the review.`
    );
  }
}
...
async function executeReviewRun(request) {
  ...
  validateReviewRequest(reviewName, focusText);
