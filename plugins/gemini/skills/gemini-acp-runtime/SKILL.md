...
- Map `spark` to `--model gemini-3.1-flash-lite`.
...
- If the forwarded request includes `--model`, normalize `spark` to `gemini-3.1-flash-lite` and pass it through to `task`.
