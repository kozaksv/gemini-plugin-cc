...
test("rescue command absorbs continue semantics", () => {
  ...
  const runtimeSkill = read("skills/gemini-acp-runtime/SKILL.md");
  ...
  // Regression for #234: `Skill(gemini:rescue)` from the main agent recursed
  // because rescue.md named the routing with ambiguous prose ("Route this
  // request to the `gemini:gemini-rescue` subagent") while running under
  ...
  assert.match(rescue, /do not call `Skill\(gemini:gemini-rescue\)`/i);
  ...
  assert.match(rescue, /If they ask for `spark`, map it to `gemini-3\.1-flash-lite`/i);
  ...
  assert.match(agent, /If the user asks for `spark`, map that to `--model gemini-3\.1-flash-lite`/i);
  assert.match(agent, /If the user asks for a concrete model name such as `gemini-2\.5-pro`, pass it through with `--model`/i);
  ...
  assert.match(runtimeSkill, /Map `spark` to `--model gemini-3\.1-flash-lite`/i);
  ...
  assert.match(readme, /if you do not pass `--model` or `--effort`, Gemini chooses its own defaults/i);
  assert.match(readme, /--model gemini-2\.5-pro --effort medium/i);
  assert.match(readme, /`spark`, the plugin maps that to `gemini-3\.1-flash-lite`/i);
...
test("setup command can offer Gemini install and still points users to authenticate", () => {
